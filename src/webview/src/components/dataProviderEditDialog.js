    ...
    (e) => {
      ...
      if (e.detail.mimeType === 'application/vnd.dlt-logs+json') {
        ...
        if (filterFrags !== undefined && Array.isArray(filterFrags) && filterFrags.length > 0) {
          ...
          if (
            (!props.applyMode && posNegFilterFrags.length > 0) ||
            (props.applyMode && (posNegFilterFrags.length > 0 || reportFilterFrags.length > 0))
          ) {
            setDataType('ext:dlt')
            // keep conv
            // keep jsonpath if not empty
            if (!props.applyMode && !dataJsonPath?.length) {
            ...
            posNegFilterFrags.forEach((frag) =>
              filterStrs.push(JSON.stringify({ ...frag, id: undefined, configs: undefined, tmpFb: props.applyMode ? 1 : undefined })),
            ) // remove id,configs, set tmpFb

            const reportStr =
              reportFilterFrags.length > 0
                ? '&report=' +
                  encodeURIComponent(
                    '[' + reportFilterFrags.map((f) => JSON.stringify({ ...f, id: undefined, configs: undefined })).join(',') + ']',
                  )
                : ''

            setDataSource(
              props.applyMode
                ? `ext:mbehr1.dlt-logs/get/docs/0/filters?delete=${encodeURIComponent('{"tmpFb":1}')}${
                    filterStrs.length > 0 ? '&disableAll=view&add=' + filterStrs.map((str) => encodeURIComponent(str)).join('&add=') : ''
                  }${reportStr}`
  ...
  const newDltQuery = (type) => {
    switch (type) {
      case 'sequence':
        ...
        if (hasEcu) {
          filters.push({ ecu: '${attributes.ecu}', name: 'selected ecu', not: true, type: 1 })
        }
        const sequences = [
          {
            name: 'sequence name',
      ...
      case 'query':
        {
          ...
          setDataSource(`ext:mbehr1.dlt-logs/get/docs/0/filters?query=${encodeURIComponent(JSON.stringify(filters, undefined, 2))}`)
        }
      ...
      <DialogContent dividers>
        <Grid container direction='column' justifyContent='flex-start' alignItems='stretch' spacing={2}>
            ...
            <Paper>
              <RadioGroup row value={dataType} onChange={(event) => setDataType(event.target.value)}>
                <FormControlLabel
                  ...
                  label={<Typography variant='body1'>https rest query</Typography>}
                />
          ...
          <Grid item>
            ...
            {dataType === 'ext:dlt' && (
              <Paper>
                ...
                <Input
                  ...
                  placeholder={
                    props.applyMode
                      ? "e.g. '/get/docs/0/filters?delete=(uriencode({...}))&add={...}'"
                      : "e.g. '/get/docs/0/filters?query=...'"
                  }
                  inputProps={{
                    value: dataSource?.startsWith('ext:mbehr1.dlt-logs') ? dataSource.slice(19) : dataSource,
                    onChange: (event) => {
                      setPreviewQueryResult(previewApplyText)
                      setPreviewBadgeStatus(props.applyMode ? 3 : 0)
                      setDataSource('ext:mbehr1.dlt-logs' + event.target.value)
                    },
                  }}
                />
              </Paper>
            ...
            {!props.applyMode && (
              <Paper>
                <InputLabel shrink htmlFor='dataJsonPathInput'>
                  Enter jsonPath expression to extract results
                </InputLabel>
                <Input
                  id='dataJsonPathInput'
                  inputComponent={OnBlurInputBase}
                  fullWidth
                  ...
                  inputProps={{
                    ...
                    onChange: (event) => {
                      setPreviewQueryResult(previewApplyText)
                      setPreviewBadgeStatus(props.applyMode ? 3 : 0)
                      setDataJsonPath(event.target.value)
                    },
                  }}
                />
                <RadioGroup
                  row
                  value={dataConv?.split(':')[0]}
                  onChange={(event) => {
                    setPreviewQueryResult(previewApplyText)
                    setPreviewBadgeStatus(props.applyMode ? 3 : 0)
                    switch (event.target.value) {
                      ...
                      case 'length':
                        ...
                        break
                      case 'func':
                        setDataConv('func:' + dataConv?.slice(dataConv.indexOf(':') + 1))
                        break
                      default:
                        break
                    }
                ...
                {dataConv?.startsWith('func') && (
                  <React.Fragment>
                    <InputLabel shrink htmlFor='dataFuncInput'>
                    ...
                    <Input
                      ...
                      inputProps={{
                        multiline: true,
                        value: dataConv.slice(dataConv.indexOf(':') + 1),
                        onChange: (event) => {
                          setPreviewQueryResult(previewApplyText)
                          setPreviewBadgeStatus(props.applyMode ? 3 : 0)
                          setDataConv('func:' + event.target.value)
                        },
                      }}
                    />
                  </React.Fragment>
          ...
          {props.applyMode && (
            <Grid item>
              <Button
                id={'dp-test-apply-filter-' + props.name}
                color='secondary'
          ...
          <Grid item>
            <Paper>
              ...
              <div>
                badge error='{previewBadgeError}' badge status='{previewBadgeStatus}'
              </div>
              <div>
