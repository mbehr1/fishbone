// copyright (c) Matthias Behr, 2020 - 2023
// based on the ...
...
export function FishboneElementUrl(props) {
  return <a href={props.href || 'href missing!'}>{props.text || 'text missing!'}</a>
}
...
export default function FishboneChart(props) {
  ...
  const effect = props?.data.length ? props.data[effectIndex] : { name: 'no effects', rootCauses: [] }
  ...
  const getColor = (index) => {
    const colors = ['blue_two', 'gray', 'black', 'green', 'blue', 'orange', 'purple', 'pink']
    ...
    const causes = rootCauses.map((rootCause, index) => {
      ...
      if (typeof rootCause === 'string') {
        ...
        switch (rootCause.type) {
          case 'Url':
            fragment = (
              <div key={`root_causes_${rootCause.title}_${index}`}>{React.createElement(FishboneElementUrl, rootCause, null)} </div>
            )
            break
          case 'react':
          ...
          case 'nested':
            ...
            fragment = (
              <Button
                style={{ marginLeft: '1px', padding: '1px 9px 1px 20px', justifyContent: 'left' }}
                fullWidth
                size='small'
                variant='outlined'
                color='primary'
                key={`root_causes_${index}`}
                onClick={() => props?.onStateChange({ childFBData: [rootCause.data, rootCause.title] })}
              >
                {rootCause.title}
              </Button>
            )
      ...
      if (props.rootCauseContextMenu && props.rootCauseContextMenu.length > 0) {
        ...
        return (
          <div style={{ height: '100%', width: '100%', display: 'flex', 'align-items': 'center' }}>
            <div style={{ position: 'relative', width: '95%' }}>{fragment}</div>
            <div style={{ position: 'relative', width: '5%' }}>
              <IconButton id={`rcMore_${keyFrag}`} onClick={handleMenuClick} color='secondary' size='small'>
                {' '}
                <MoreVertIcon fontSize='small' />
              </IconButton>
              <Menu
                key={`cm_root_causes_${keyFrag}`}
                anchorEl={menuAnchorEl}
                keepMounted
                open={menuOpen === `rcMore_${keyFrag}`}
                onClose={handleMenuClose}
              >
                {props.rootCauseContextMenu
                  .filter((m) => m !== undefined)
                  .map((menuItem, index) => (
                    <MenuItem
                      onClick={(event) => {
                        handleMenuClose()
                        menuItem?.cb(props.data, effectIndex, category, rootCause)
                      }}
                    >
                      {menuItem.text}
                    </MenuItem>
          ...
          <div key={`top_causes_${category.name}_${index}`} className='causeContent'>
            <div className={`cause top ${color}_ ${color}Border`}>
              <OnBlurInputBase
                ...
                margin='dense'
                value={category.name}
                onChange={(event) => props.onChange(category, event, 'name')}
              />
              {props.categoryContextMenu && props.categoryContextMenu.length > 0 /* todo race cond if all items are undefined... */ && (
                <React.Fragment>
                  ...
                  <Menu
                    ...
                    anchorEl={menuAnchorEl}
                    keepMounted
                    open={menuOpen === `top_cat_${index}`}
                    onClose={handleMenuClose}
                  >
                    {props.categoryContextMenu
                      .filter((m) => m !== undefined)
                      .map((menuItem, index) => (
                        <MenuItem
                          onClick={() => {
                            handleMenuClose()
                            menuItem?.cb(props.data, effectIndex, category)
                          }}
                        >
                          {menuItem.text}
                        </MenuItem>
              ...
              {props.categoryContextMenu && props.categoryContextMenu.length > 0 && (
                <React.Fragment>
                  ...
                  <Menu
                    ...
                    anchorEl={menuAnchorEl}
                    keepMounted
                    open={menuOpen === `bottom_cat_${index}`}
                    onClose={handleMenuClose}
                  >
                    {props.categoryContextMenu
                      .filter((m) => m !== undefined)
        ...
        {props.effectContextMenu && props.effectContextMenu.length > 0 && (
          <React.Fragment>
            ...
            <Menu anchorEl={menuAnchorEl} keepMounted open={menuOpen === `effect_${effectIndex}`} onClose={handleMenuClose}>
              {props.effectContextMenu
                .filter((e) => e !== undefined)
                .map((menuItem, index) => (
                  <MenuItem
                    onClick={() => {
                      handleMenuClose()
                      menuItem?.cb(props.data, effectIndex)
                    }}
                  >
                    {menuItem.text}
                  </MenuItem>
