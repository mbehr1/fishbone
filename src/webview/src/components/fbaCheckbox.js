...
export default function FBACheckbox(props) {
  ...
  useEffect(() => {
    ...
    if ((!badgeStatus && values.badge?.source) || (badgeStatus > 0 && badgeQuery !== bs)) {
      const fetchdata = async () => {
        try {
          setBadgeStatus(1)
          setBadgeQuery(bs)
  ...
  useEffect(() => {
    ...
    if ((badge2Status === 0 && values.badge2?.source) || (badge2Status > 0 && badge2Query !== b2s)) {
      const fetchdata = async () => {
        try {
          setBadge2Status(1)
          setBadge2Query(b2s)
  ...
  const backgroundFragments = (
    <React.Fragment>
      <Accordion defaultExpanded>
        <AccordionSummary expandIcon={<ExpandMoreIcon />} id='backgroundDescription-textfield-header'>
        ...
        <AccordionDetails>
          <TextFieldEditDialog
            ...
            data={values.backgroundDescription || {}}
            onChange={(newValue) => handleValueChanges({ target: { name: 'backgroundDescription', value: newValue } })}
            open={textFieldEditOpen === 1}
            onClose={() => {
              setTextFieldEditOpen(0)
            }}
          />
          <RenderConditionText
            markdownActive={GetMarkdownActive(values.backgroundDescription)}
            text={GetTextValue(values.backgroundDescription)}
          />
        </AccordionDetails>
      </Accordion>
    </React.Fragment>
  ...
  const instructionsFragment = (
    <React.Fragment>
      <Accordion defaultExpanded>
        <AccordionSummary expandIcon={<ExpandMoreIcon />} id='instructions-textfield-header'>
          <Typography gutterBottom variant='h6'>
            Instructions
          </Typography>
          <IconButton
        ...
        <AccordionDetails>
          <TextFieldEditDialog
            ...
            data={values.instructions || {}}
            onChange={(newValue) => handleValueChanges({ target: { name: 'instructions', value: newValue } })}
            open={textFieldEditOpen === 2}
            onClose={() => {
              setTextFieldEditOpen(0)
            }}
          />
  ...
  const processingCommentsFragment = (
    <React.Fragment>
      <Accordion defaultExpanded>
        <AccordionSummary expandIcon={<ExpandMoreIcon />} id='background-textfield-header'>
          <Typography gutterBottom variant='h6'>
            Comments
          </Typography>
          <IconButton
        ...
        <AccordionDetails>
          <TextFieldEditDialog
            ...
            onChange={(newValue) => handleValueChanges({ target: { name: 'comments', value: newValue } })}
            open={textFieldEditOpen === 3}
            onClose={() => {
              setTextFieldEditOpen(0)
            }}
          />
  ...
  const applyFilterFragment = values.filter && (
    <React.Fragment>
      <div>
        <Snackbar
          id={'snackbar-apply-filter-' + props.name}
          open={applyFilterBarOpen}
          autoHideDuration={6000}
          anchorOrigin={{ vertical: 'bottom', horizontal: 'right' }}
          onClose={handleFilterBarClose}
        >
          <Alert
            id={'alert-snackbar-apply-filter-' + props.name}
            variant='filled'
            onClose={handleFilterBarClose}
            severity='info'
            sx={{ width: '100%' }}
        ...
        <Button
          id={'apply-filter-' + props.name}
          size='small'
          color='secondary'
          startIcon={<FilterListIcon />}
          onClick={(e) => handleApplyFilter(values.filter)}
        >
          Apply filter
        </Button>
      </div>
  ...
  const contentTooltip = (content, maxLen) => {
    if (typeof content === 'number') {
      ...
      try {
        ...
        if (strLen > 0) {
          return (
            <Tooltip title={<span style={{ whiteSpace: 'pre-line' }}>{str}</span>} arrow>
      ...
      <Box py='1px' /* py = padding-top+bottom <- padding around checkbox */>
        <Grid container spacing={1} m={'-4px'} /* distance between checkbox and edit icon */>
          <Grid item flex style={{ padding: '1px' }}>
            <Badge
              classes={{ badge: classes.upperLeftBadge }}
              badgeContent={contentTooltip(numberAbbrev(badgeCounter, 999), 40)}
              color='error'
              anchorOrigin={{ vertical: 'top', horizontal: 'left' }}
              overlap='circular'
              max={NaN}
              invisible={values.value === 'ok' || badgeStatus < 2 || (typeof badgeCounter === 'number' && badgeCounter === 0)}
            >
              <Badge
                classes={{ badge: classes.lowerRightBadge }}
                badgeContent={contentTooltip(numberAbbrev(badge2Counter, 99), 40)}
                max={NaN}
                color={undefined}
                anchorOrigin={{ vertical: 'bottom', horizontal: 'right' }}
                overlap='circular'
                invisible={values.value === 'ok' || badge2Status < 2}
                showZero
              >
                <MultiStateBox
                  values={[
                    { value: null, icon: <CheckBoxOutlineBlankIcon fontSize='small' /> },
                    { value: 'ok', icon: <CheckBoxIcon fontSize='small' /> },
                    { value: 'error', icon: <ErrorIcon fontSize='small' style={{ color: '#f44336' }} /> },
                  ]}
                  {...props}
                  size='small'
                  color='primary'
                />
                <FormControlLabel
                  style={{ flex: 'auto', 'min-width': '234px', margin: '0px', justifyContent: 'space-between' }}
                  ...
                  control={
                    <IconButton
                      ...
                      style={{ padding: '4px 0px 2px 0px', margin: '0px' }}
                    >
                      <EditIcon fontSize='small' />
                    </IconButton>
                  }
